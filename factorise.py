...
def main():
	...
	#initialise primes list	
	primes=[]

	# Want to limit highest prime imported from csv to be <= math.sqrt(N)	
	primes=csvfile_store_primes(primefile,floor_sqrt_N)

	# Now call factorise() function
	result=factorise(N,floor_sqrt_N)
...
def calc_primes_powers_remainder(primes,N_remainder):
	...
	if not primes:	
		...
		#print("There are no elements in prime list??")		
...
def csvfile_store_primes(csv_filename_var,floor_sqrt_N):
