...
def main():
	...
	#primes=csvfile_store_primes(primefile,floor_sqrt_p, p)
	...
	if math.sqrt(p) > primes[-1]:
		print('Square root of p is greater than largest prime in list - '+str(primes[-1])+'. Consider using a larger prime list. Exiting..')
		sys.exit()
...
def euler_phi(n):
	...
	#euler_phi(9) = 6, since gcd of 9 with the following integers is 1: 1,2,4,5,7 and 8. 
...
def dlp(g, h, p, primes):
	...
	for C in C_list:
		...
		print('Now need to solve for each x...')
